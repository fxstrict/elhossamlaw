 ...
 * ================================================================
 * PHASE 16.13 — DomRecycler Migration (Fees) — Verification
 * ================================================================
 * Standalone Node harness (`node js/tests/verify_fees_dom_recycler.js`,
 * no browser required). Loads the REAL js/modules/fees.js (via the
 * same vm/Module.wrap technique verify_fees_repository_integration.js
 * and verify_sessions_dom_recycler.js already use) against a REAL
 * FeesRepository (backed by the fake localStorage/indexedDB doubles
 * every verify_*_repository*.js harness uses) and the REAL
 * js/core/dom/{DomKeyIndex,DomNodeFactory,DomPatch,DomRecycler}.js
 * files — exercising renderFees() end-to-end against a full fake DOM
 ...
 *
 * PHASE 16.13 REVIEW NOTE (see handoff report): renderFees() uses TWO
 * containers — #feesTableBody (<tr>, no className) and #feesMobileList
 * (<div class="m-card">) — the exact same dual-container shape as
 * renderCases() (PHASE 16.6), NOT the single-container shape of
 * Sessions/Documents/Clients. This harness therefore exercises BOTH
 * containers on every render, following the pattern this project would
 * use for a Cases-shaped test (no dedicated verify_cases_dom_recycler.js
 * exists to copy directly, so the dual-container assertions below are
 * built from first principles against the SAME DomRecycler/DomKeyIndex/
 * DomPatch/DomNodeFactory contract every other verify_*_dom_recycler.js
 * harness already exercises).
 *
 * No production file is modified by this harness. It is read-only with
 * respect to js/modules/fees.js and js/core/dom/*.
 *
 * This does NOT re-test DomRecycler.reconcile()'s own internal
 * mechanics (already covered by verify_dom_recycler.js) — it tests
 * renderFees() itself: that it wires reconcile() correctly for BOTH
 * containers, with the correct key/tag/className/render, and falls
 * back correctly.
 *
 * Coverage (per Phase 16.13 handoff prompt, item "الاختبارات المطلوبة"):
 *   A. Initial Render        F. Filter
 *   B. Reuse                 G. Reorder (N/A — no sort in renderFees())
 ...
 * verify_fees_repository_integration.js's / verify_sessions_dom_recycler.js's
 * loadModule().
 */
...
async function main() {
  console.log('================================================================');
  console.log('PHASE 16.13 — DomRecycler Migration (Fees) — Verification');
  console.log('================================================================\n');
  ...
  //      index.html's <script> tags would, so fees.js's
  ...
  // ---- Fake elements the rest of fees.js reads via val()/getElementById ----
  const fakeElements = {};
  function getEl(id) {
  ...
  // feesTableBody / feesMobileList must be the SAME live containers
  // across every render() call in this harness (getEl() caches them),
  // exactly matching how a real page keeps one #feesTableBody /
  // #feesMobileList element across repeated renderFees() calls.
  const tb = getEl('feesTableBody');
  const ml = getEl('feesMobileList');
  getEl('feesEmpty');
  getEl('feesTotalNum');
  getEl('feesCountNum');

  async function makeFee(overrides) {
    const base = {
      'رقم_القضية': '2026/100',
      'اسم_الموكل': 'موكل',
      'نوع_الأتعاب': 'أتعاب محاماة',
      'المبلغ': '1000',
      'تاريخ_الاستلام': '2026-08-01',
      'طريقة_الدفع': 'نقدي'
    };
    return feesModule.feesRepository.create(Object.assign(base, overrides));
  }

  // ---- A. Initial Create -------------------------------------------------
  await asyncSection('A. Initial render creates one <tr>/one m-card per fee, with correct className on both containers', async () => {
    await makeFee({ 'اسم_الموكل': 'أحمد علي', 'المبلغ': '1000' });
    await makeFee({ 'اسم_الموكل': 'سارة محمود', 'المبلغ': '2000' });
  ...
  // ---- C. Content Update ---------------------------------------------------
  await asyncSection('C. Editing a fee field updates content on the SAME node in both containers', async () => {
    const rec = feesModule.feesRepository.getAll()[0];
    const rowBefore = tb.children[0];
    const cardBefore = ml.children[0];

    resetWriteCounts();
    await feesModule.feesRepository.update(rec[feesModule.FEES_ID_FIELD], { 'طريقة_الدفع': 'تحويل بنكي' });
    feesModule.renderFees();
    ...
    countingAssert(tb.children[0].innerHTML.indexOf('تحويل بنكي') !== -1, 'updated payment method present in row');
    countingAssert(DOM_WRITE_COUNTS.attrs === 0, 'fees has no attrs() layer — no attribute writes expected, got ' + DOM_WRITE_COUNTS.attrs);
  });

  // ---- D. Delete ------------------------------------------------------------
  await asyncSection('D. Deleting a fee removes exactly its node from both containers', async () => {
    const all = feesModule.feesRepository.getAll();
    const idToDelete = all[0][feesModule.FEES_ID_FIELD];
    const survivingClient = all[1]['اسم_الموكل'];

    await feesModule.feesRepository.delete(idToDelete);
    feesModule.renderFees();
    ...
    countingAssert(tb.children[0].innerHTML.indexOf(survivingClient) !== -1, 'the surviving row is the correct one');
    countingAssert(ml.children[0].innerHTML.indexOf(survivingClient) !== -1, 'the surviving card is the correct one');
  });

  // ---- E. Insert --------------------------------------------------------------
  await asyncSection('E. Adding a new fee creates exactly one new node in each container, existing nodes untouched', async () => {
    const rowBefore = tb.children[0];
    const cardBefore = ml.children[0];
    resetWriteCounts();
    await makeFee({ 'اسم_الموكل': 'محمد سعيد', 'المبلغ': '3000' });
    feesModule.renderFees();
  ...
  await asyncSection('F. Free-text search narrows and restores the visible set correctly on both containers', async () => {
    fakeElements['searchFees'] = new FakeElement('input');
    fakeElements['searchFees'].value = 'محمد سعيد';
    feesModule.renderFees();
  ...
  // ---- G. Reorder — N/A ---------------------------------------------------
  section('G. Reorder — not applicable (renderFees() applies no sort; documented, not a gap)', () => {
    // FeesRepository.search() applies no sort unless queryModel.sort is
    // explicitly passed (which renderFees() never does — see file header
    // "SEARCH" note), so row order always matches insertion order. There
    // is therefore no reorder path for DomPatch.position()'s move branch
    // to exercise here, unlike Sessions (sorted by التاريخ). This is a
    // pre-existing, documented behavior of Fees, not a migration gap.
  ...
  await asyncSection('H. Emptying the visible set clears both containers and shows the empty-state element', async () => {
    ...
    for (const rec of all) {
      await feesModule.feesRepository.delete(rec[feesModule.FEES_ID_FIELD]);
    }
    feesModule.renderFees();

    countingAssert(tb.children.length === 0, 'table cleared when no fees match');
    countingAssert(ml.children.length === 0, 'mobile list cleared when no fees match');
    countingAssert(fakeElements['feesEmpty'].style.display === '', 'empty-state element shown');
  });

  // ---- I. Fallback -----------------------------------------------------------
  await asyncSection('I. If DomRecycler.reconcile() throws, renderFees() falls back to a correct full rebuild on both containers', async () => {
    await makeFee({ 'اسم_الموكل': 'فاطمة كريم', 'المبلغ': '4500' });
    ...
    countingAssert(tb.children.length === 0, 'legacy fallback assigns a raw HTML string, not live child nodes, in this harness (matches verify_dom_recycler.js test I / verify_sessions_dom_recycler.js test H)');
    countingAssert(ml.children.length === 0, 'same for the mobile list container');
    countingAssert(tb.innerHTML.indexOf('فاطمة كريم') !== -1, 'fallback table output still contains fee content');
    countingAssert(ml.innerHTML.indexOf('m-card') !== -1, 'fallback mobile output still contains the m-card wrapper');
    countingAssert(ml.innerHTML.indexOf('فاطمة كريم') !== -1, 'fallback mobile output still contains fee content');
  ...
  await asyncSection('J. A later re-add after emptying creates fresh nodes in both containers (no stale key-index carried across)', async () => {
    ...
    await makeFee({ 'اسم_الموكل': 'خالد إبراهيم', 'المبلغ': '5000' });
    resetWriteCounts();
    feesModule.renderFees();
    ...
    countingAssert(fakeElements['feesEmpty'].style.display === 'none', 'empty-state element hidden again');
  });
